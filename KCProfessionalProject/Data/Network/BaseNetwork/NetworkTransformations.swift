...
//                    print("urlhero:\(url) requestHero\(request), filter: \(filter), name: \(modelReturn)")
    ...
    func getTransformations(heroId: String) async -> [TransformationsModel] {
        ...
        var request: URLRequest = URLRequest(url: URL(string: url)!)
        request.httpMethod = HTTPMethods.post
        request.httpBody = try? JSONEncoder().encode(TransformationModelRequest(id: heroId)) // Cuerpo de la solicitud con el ID del héroe
        request.addValue(HTTPMethods.content, forHTTPHeaderField: "Content-type")
        ...
        do {
            ...
            if let resp = response as? HTTPURLResponse {
                ...
                print("CODIGO DE STATUS\(resp.statusCode)")
            } else {
